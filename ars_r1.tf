...
resource "azurerm_route_server_bgp_connection" "inject-vm-2" {
  name            = "vm-inject-2"
  peer_asn        = var.asn_inject
  peer_ip         = module.r1_inject_vm_2.vm_private_ip
  route_server_id = module.ars_r1.ars.id
  depends_on      = [azurerm_route_server_bgp_connection.inject-vm]
}
...
data "template_file" "cloudconfig-inject" {
  ...
  vars = {
    ...
    peer_ilb_2_ip_address = azurerm_lb.fw_lb_2.private_ip_address
    asn_inject            = var.asn_inject
    asn_transit           = var.asn_transit
    spoke_vnet_cidr       = azurerm_subnet.spoke-vm-subnet.address_prefixes[0]
    spoke_2_vnet_cidr     = azurerm_subnet.spoke-2-vm-subnet.address_prefixes[0]
